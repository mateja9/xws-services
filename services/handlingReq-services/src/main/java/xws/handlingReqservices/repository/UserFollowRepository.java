...
public interface UserFollowRepository extends CrudRepository<UserFollow, Long> {
    ...
    @Query("SELECT count(u) FROM UserFollow u WHERE u.userToId = :userId AND u.isActive = false AND u.status = 'accepted'")
    Integer countFollowers(@Param("userId") Long userId);

    @Query("SELECT count(u) FROM UserFollow u WHERE u.userFromId = :userId AND u.isActive = false AND u.status = 'accepted'")
